...
function sanitizeDebugRoomPassword(value: string | undefined) {
  return (value ?? '').trim()
}
...
function sanitizeCounterKind(value: string) {
  const normalized = value.trim().toLowerCase().replace(/[^a-z0-9/+ -]/g, '')
  return normalized.slice(0, 20)
}

function clampCounterDelta(value: number) {
  return Math.max(-20, Math.min(20, Math.trunc(value)))
}

function clampDrawAmount(value: number | undefined) {
  return Math.max(1, Math.min(7, Math.trunc(value ?? 1)))
}

function clampTurnNumber(value: number | undefined, fallback = 1) {
  return Math.max(1, Math.min(999, Math.trunc(value ?? fallback)))
}

function isCommanderCandidate(card: MagicCard) {
  return (
    card.typeLine.includes('Legendary Creature') || card.typeLine.includes('Legendary Planeswalker')
  )
}

function hasPartnerText(card: MagicCard) {
...
function isVirtualTokenCard(cardId: string) {
...
function sanitizePlayerNote(value: string | undefined) {
...
function sanitizeTargets(value: string[] | undefined) {
  return (value ?? []).map((entry) => entry.trim()).filter(Boolean).slice(0, 6)
}

function sanitizePlayerCounterKind(value: string) {
  return sanitizeCounterKind(value)
}

function buildDefaultPlayerDesignations(): PlayerDesignations {
  return {
    monarch: false,
    initiative: false,
    citysBlessing: false,
...
function buildHiddenCard(card: MagicCard): MagicCard {
  return {
    ...card,
    name: 'Face-down card',
    manaCost: '',
...
function buildDebugDeckSelection(playerName: string): DeckSelectionSnapshot {
...
export class PlayServer {
  ...
  private readonly debugUnlockedSessions = new Set<string>()
  ...
  private unlockDebugMode(sessionId: string, password: string) {
  ...
  private createDebugRoom(sessionId: string, settings: RoomSettingsInput | undefined) {
  ...
  private addDebugPlayer(sessionId: string, requestedRoomId: string, name?: string) {
    const room = this.getRoomBySession(sessionId)
  ...
  private removeDebugPlayer(sessionId: string, requestedRoomId: string, playerId: string) {
    const room = this.getRoomBySession(sessionId)
    ...
    const debugPlayerIndex = room.players.findIndex(
      (player) => player.id === playerId && player.sessionId === null,
    )
    ...
    switch (action.type) {
      ...
      case 'set_active_player': {
        ...
        this.recordEvent(
          ...
          actor.id,
          action.type,
          `${actor.name} set ${targetPlayer.name} as the active player.`,
        )
        break
      }
      ...
      case 'untap_all': {
        const tappedPermanents = game.battlefield.filter(
          (card) => card.controllerPlayerId === actor.id && card.tapped,
        )
      ...
      case 'adjust_player_counter': {
        ...
        this.recordEvent(
          ...
          actor.id,
          action.type,
          `${actor.name} adjusted ${targetPlayer.name}'s ${counterKind} counter by ${
            normalizedDelta > 0 ? `+${normalizedDelta}` : normalizedDelta
          }.`,
        )
        break
      }
      ...
      case 'set_player_designation': {
        ...
        this.recordEvent(
          ...
          actor.id,
          action.type,
          `${actor.name} ${action.value ? 'gave' : 'cleared'} ${
            action.designation === 'citys_blessing'
              ? "city's blessing"
              : `the ${action.designation}`
          } ${action.value ? 'to ' : 'from '}${targetPlayer.name}.`,
        )
        break
      }
      ...
      case 'adjust_commander_tax': {
        ...
        this.recordEvent(
          ...
          actor.id,
          action.type,
          `${actor.name} adjusted ${targetPlayer.name}'s commander tax by ${
            normalizedDelta > 0 ? `+${normalizedDelta}` : normalizedDelta
          }.`,
        )
        break
      ...
      case 'adjust_commander_damage': {
        ...
        const existingEntry = targetPlayer.commanderDamage.find(
          (entry) => entry.sourcePlayerId === sourcePlayer.id,
        )
        ...
        this.recordEvent(
          ...
          actor.id,
          action.type,
          `${actor.name} adjusted commander damage from ${sourcePlayer.name} to ${targetPlayer.name} by ${
            normalizedDelta > 0 ? `+${normalizedDelta}` : normalizedDelta
          }.`,
        )
        break
      ...
      case 'adjust_permanent_counter': {
        ...
        this.recordEvent(
          ...
          actor.id,
          action.type,
          `${actor.name} adjusted ${counterKind} counters on ${permanent.card.name} by ${normalizedDelta > 0 ? `+${normalizedDelta}` : normalizedDelta}.`,
        )
        break
      }
      ...
      case 'set_permanent_face_down': {
        ...
        this.recordEvent(
          ...
          actor.id,
          action.type,
          `${actor.name} turned ${action.faceDown ? permanent.card.name : 'a face-down card'} ${
            action.faceDown ? 'face down' : 'face up'
          }.`,
        )
        break
      }
      ...
      case 'change_control': {
        ...
        this.recordEvent(
          ...
          actor.id,
          action.type,
          `${actor.name} moved control of ${permanent.card.name} to ${targetPlayer.name}.`,
        )
        break
      }
      ...
      case 'create_stack_item': {
        ...
        let sourceZone: OwnedZone | null = null
        ...
        this.recordEvent(
          ...
          actor.id,
          action.type,
          `${actor.name} added ${sanitizeLabel(action.label, fallbackLabel)} to the stack.`,
        )
        break
      }
      ...
      case 'remove_stack_item': {
        ...
        if (stackItem.sourceCard) {
          const fallbackZone =
            stackItem.itemType === 'spell' &&
            /Artifact|Creature|Enchantment|Land|Planeswalker|Battle/i.test(
              stackItem.sourceCard.card.typeLine,
            )
              ? 'battlefield'
              : 'graveyard'
          ...
          this.placeOwnedCard(
            game,
            actor,
            action.toZone ?? fallbackZone,
            stackItem.sourceCard,
            action.position,
          )
        }
        ...
        this.recordEvent(
          ...
          action.type,
          `${actor.name} ${
            action.type === 'resolve_stack_item' ? 'resolved' : 'removed'
          } ${stackItem.label} from the stack.`,
        )
        break
      }
      ...
      case 'create_token': {
        ...
        const tokenCard = buildTokenCard(
          actor.id,
          tokenName,
          tokenType,
          tokenColors,
        ...
        this.recordEvent(
          ...
          actor.id,
          action.type,
          `${actor.name} created ${power && toughness ? `${power}/${toughness} ` : ''}${tokenName}.`,
        )
        break
      }
    }
  ...
  private getNextPlayer(game: GameState, activePlayerId: string) {
  ...
  private getControllablePermanent(game: GameState, actor: GamePlayerState, cardId: string) {
  ...
  private toStackItemSnapshot(item: StackItemState, viewerPlayerId: string): StackItemSnapshot {
    const canViewCard =
      !item.faceDown ||
  ...
  private createDebugPlaceholderPlayer(seatNumber: number, name?: string): RoomPlayerState {
